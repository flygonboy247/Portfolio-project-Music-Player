...
- **Due Date**: <!-- TODO: fill out with due date and time (e.g., 10/17 @ 3:10 PM EST) and delete this comment -->3/22
...
In the secondary I added size() which nessesitated me to add getSize in kernal
I also added and implimented toString() and equals() for toString using getSong
made it pretty simple only needing to use getSize to make a loop to print out
all the song data. For equals, I used the hashcode from standard and checked the
current and in question Song list for euqlity between each Objects.Finally I
implimented the shuffle method that shuffles the list semi randomly using a
formula that should move all data around by removing a song at a semi random
place and adding it to the back of the data structure.
